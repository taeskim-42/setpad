// Run synthetic questions through the real model and record the complete plan.
// Grade exact dates, absent filters and operations using independent labels.
// These synthetic cases are not a blind real-user accuracy benchmark.
//
...
Future<void> main() async {
  ...
  for (final q in questions) {
    ...
    final errors = exp == null
        ? null
        : gradeRecordQuestion(out, exp); // null = 관찰만
    if (exp != null && errors != null) {
      tally.putIfAbsent(exp['cat'] as String, () => []).add(errors.isEmpty);
    }
    debugPrint(
      'INTERPRET ${jsonEncode({'question': q, 'ms': watch.elapsedMilliseconds, 'cat': ?exp?['cat'], 'ok': ?errors?.isEmpty, 'errors': ?errors, ...out})}',
      wrapWidth: 10000,
    );
  }
  ...
  debugPrint(
    'INTERPRET_SUMMARY ${jsonEncode({
      ...
      'graded': all.length,
      'byCategory': {for (final e in tally.entries) e.key: '${e.value.where((b) => b).length}/${e.value.length}'},
    })}',
    wrapWidth: 10000,
  );
  debugPrint('INTERPRET_DONE');
