...
Map _withStatedMetricFor(String action, String q) {
  final plan = RecordQueryPlan.decode(
    ...
    const ['스쿼트', '벤치프레스', '데드리프트'], question: q,
  );
  // decode 는 action 을 metric 으로 바꿔 돌려준다. 되돌려 견준다.
  const back = {'max': 'heaviest', 'reps': 'repCount', 'sets': 'setCount',
    'sessions': 'trainingDays', 'trend': 'weightHistory', 'last': 'latest',
    'volume': 'volume', 'average': 'meanWeight'};
...
void main() {
  ...
  group('글이 정하는 것', () {
    ...
    test('달 이름과 최근 N주·개월', () {
      ...
      expect((sept.period, sept.since, sept.until), ('custom', '2026-09-01', '2026-09-30'));
    ...
    test('단위가 회인 무게 조건은 버리고, 글의 회수 조건이 채운다', () {
      final p = RecordQueryPlan.decode(
        {'action': 'setCount', 'exercises': ['벤치프레스'], 'periods': ['all'],
         'weight': {'relation': 'atLeast', 'value': 5, 'unit': '회'}},
        names, question: '벤치 세트 몇 개 5회 이상',
      );
      final r = p.requests.single;
    ...
    test('글에 기간이 하나면 모델의 비교 기간 둘은 접는다', () {
      final t = DateTime(2026, 9, 9);
      final p = RecordQueryPlan.decode(
        {'action': 'weightHistory', 'exercises': ['벤치프레스'],
         'periods': ['lastMonth', 'thisMonth']},
        names, today: t, question: '이번 달 벤치 무게 변화',
      );
    ...
    test('운동 하나를 지목한 순위는 의도 낱말로 바꾼다', () {
      final p = RecordQueryPlan.decode(
        {'action': 'rankExercises', 'metric': 'trainingDays', 'limit': 1,
         'exercises': ['벤치프레스'], 'periods': ['all']},
        names, question: '벤치프레스는 정체기인가',
      );
      expect(p.rank, isFalse);
    ...
    test('이름이 필요한데 * 면, 글에서 하나만 잡힐 때 그걸로', () {
      final p = RecordQueryPlan.decode(
        {'action': 'weightHistory', 'exercises': [], 'periods': ['all']},
        names, question: '스쿼트 요즘 어때',
      );
      expect(p.requests.single.exercise, '스쿼트');
